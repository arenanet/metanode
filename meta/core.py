...
class Register(type):
    """Metatype for tracking all Metanode classes in the import path."""
    __meta_types__ = {}
...
def deserialize_metanode(data, node=None, json_format=True):
    """
    ...
    # Regular metanodes need to either use the given node or create a new one
        ...
        if node is None:
            metanode = metanode_class.create(node_name)
        else:
